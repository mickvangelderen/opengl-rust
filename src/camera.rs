...
impl<T> Clamp for T where T: PartialOrd, T: Copy {
    fn clamp(&self, min: Self, max: Self) -> Self {
        if *self > max {
            max
        } else if *self < min {
...
impl Camera {
    pub fn update(&mut self, update: &CameraUpdate) {
        ...
        let new_fov = self.fov + Rad(update.delta_scroll) * self.zoom_velocity * update.delta_time;

        // Apply updates.
        self.position = new_position;
        self.yaw = new_yaw % Rad::full_turn();
        self.pitch = new_pitch.clamp(Rad::from(Deg(-89.0)), Rad::from(Deg(89.0)));
        self.fov = new_fov.clamp(Rad::from(Deg(10.0)), Rad::from(Deg(80.0)));
    }


    pub fn orientation(&self) -> Quaternion<f32> {
        Quaternion::from_axis_angle(Vector3::unit_y(), -self.yaw) * Quaternion::from_axis_angle(Vector3::unit_x(), -self.pitch)
    }

    pub fn pos_from_wld_to_cam_space(&self) -> Matrix4<f32> {
